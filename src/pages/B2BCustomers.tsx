...
export default function B2BCustomers({ onBack }: Props) {
  ...
  async function saveParsedInvoice(idx: number) {
    ...
    if (inv.invoice_number) {
      ...
      if (existing) {
        ...
        const updRes = await safeDbOperation(
          () => supabase.from('b2b_invoices').update({ customer_id: customerId, invoice_date: dateDb, due_date: dueDateStr, total_before_vat: Number(inv.total_before_vat) || 0, total_with_vat: Number(inv.total_with_vat) || Number(inv.total_before_vat) * VAT_DIVIDER || 0 }).eq('id', existing.id),
          'עדכון חשבונית קיימת',
        )
    ...
    const invRes = await safeDbOperation(
      () => supabase.from('b2b_invoices').insert({ customer_id: customerId, invoice_number: inv.invoice_number || null, invoice_date: dateDb, due_date: dueDateStr, total_before_vat: amount, total_with_vat: Number(inv.total_with_vat) || amount * VAT_DIVIDER, branch_id: branchId, status: 'open', doc_type: docType, uploaded_by: appUser?.name }),
      'יצירת חשבונית B2B',
    )
  ...
  async function confirmCreateCustomerAndSave(idx: number, customerName: string) {
    ...
    const docType = inv.doc_type === 'delivery_note' ? 'delivery_note' : 'invoice'
    const invRes = await safeDbOperation(
      () => supabase.from('b2b_invoices').insert({ customer_id: newCust.id, invoice_number: inv.invoice_number || null, invoice_date: dateDb, due_date: dueDateStr, total_before_vat: amount, total_with_vat: Number(inv.total_with_vat) || amount * VAT_DIVIDER, branch_id: branchId, status: 'open', doc_type: docType, uploaded_by: appUser?.name }),
      'יצירת חשבונית B2B',
    )
            ...
            {showAddInv && parsedPdfs.length === 0 && (
              <div style={{ background: '#f8fafc', borderRadius: 10, padding: 16, marginBottom: 16, border: '1px solid #f1f5f9' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 10 }}>
                  ...
                  <div><label style={S.label}>סה"כ לפני מע"מ</label><input type="number" value={invForm.total_before_vat} onChange={e => setInvForm(p => ({ ...p, total_before_vat: e.target.value, total_with_vat: String(Number(e.target.value) * VAT_DIVIDER) }))} style={S.input} /></div>
                  ...
                  <tbody>{parsedPdfs.map((p, i) => (
                    <tr key={i} style={{ background: p.status === 'error' ? '#fef2f2' : p.status === 'saved' ? '#f0fdf4' : !p.selected ? '#f8fafc' : i % 2 === 0 ? 'white' : '#fafbfc', opacity: p.status === 'parsed' && !p.selected ? 0.5 : 1, textDecoration: p.status === 'parsed' && !p.selected ? 'line-through' : 'none' }}>
                      ...
                      : (<>
                        ...
                        <td style={{ ...S.td, fontWeight: 600 }}><input type="number" step="0.01" value={p.total_before_vat || ''} onChange={e => setParsedPdfs(prev => prev.map((pp, j) => j === i ? { ...pp, total_before_vat: Number(e.target.value), total_with_vat: Number(e.target.value) * VAT_DIVIDER } : pp))} style={{ ...S.input, padding: '4px 8px', fontSize: 12, width: 80 }} /></td>
