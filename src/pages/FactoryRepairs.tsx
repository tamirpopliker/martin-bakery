    ...
    <div className="min-h-screen bg-slate-100" style={{ direction: 'rtl' }}>
      ...
      <div className="page-container" style={{ padding: '32px', maxWidth: '900px', margin: '0 auto' }}>
        <Card className="shadow-sm" style={{ marginBottom: '24px' }}>
          <CardContent className="p-6">
            ...
            <button onClick={handleAdd} disabled={loading || !amount}
              style={{ marginTop: '16px', background: loading || !amount ? '#e2e8f0' : '#fbbf24', color: loading || !amount ? '#94a3b8' : 'white', border: 'none', borderRadius: '10px', padding: '10px 28px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Plus size={18} />הוסף
            </button>
          </CardContent>
        </Card>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <div style={{ background: '#fbbf24', color: 'white', borderRadius: '10px', padding: '8px 20px', fontWeight: '700', fontSize: '15px' }}>
                ...
                <div key={entry.id} style={{ display: 'grid', gridTemplateColumns: '120px 120px 1fr 130px 36px 36px', alignItems: 'center', padding: '14px 24px', borderBottom: i < entries.length - 1 ? '1px solid #f1f5f9' : 'none', background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                  {editId === entry.id ? (
                    <>
                      ...
                      <select value={editData.type || ''} onChange={e => setEditData({ ...editData, type: e.target.value })} style={{ border: '1px solid #818cf8', borderRadius: '6px', padding: '4px 8px', fontSize: '13px' }}>
                        {types.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                      </select>
                    ...
                    <>
                      ...
                      <span style={{ fontSize: '13px', background: '#fffbeb', color: '#fbbf24', padding: '3px 10px', borderRadius: '20px', fontWeight: '600', display: 'inline-block' }}>
                        {types.find(t => t.value === entry.type)?.label || entry.type}
                      </span>
                      <span style={{ fontSize: '13px', color: '#64748b' }}>{entry.description || '—'}</span>
                      <span style={{ fontWeight: '700', color: '#fbbf24', fontSize: '15px' }}>₪{Number(entry.amount).toLocaleString()}</span>
