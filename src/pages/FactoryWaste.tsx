      ...
      <div style={{ padding: '32px', maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ background: 'white', borderRadius: '20px', padding: '28px', marginBottom: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          ...
          <button onClick={handleAdd} disabled={loading || !amount}
            style={{ marginTop: '16px', background: loading || !amount ? '#e2e8f0' : '#ef4444', color: loading || !amount ? '#94a3b8' : 'white', border: 'none', borderRadius: '10px', padding: '10px 28px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div key={entry.id} style={{ display: 'grid', gridTemplateColumns: '120px 140px 1fr 130px 36px 36px', alignItems: 'center', padding: '14px 24px', borderBottom: i < entries.length - 1 ? '1px solid #f1f5f9' : 'none', background: i % 2 === 0 ? 'white' : '#fafafa' }}>
              {editId === entry.id ? (
                <>
                  ...
                  <select value={editData.category || ''} onChange={e => setEditData({ ...editData, category: e.target.value })} style={{ border: '1px solid #3b82f6', borderRadius: '6px', padding: '4px 8px', fontSize: '13px' }}>
                    {categories.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                  </select>
                ...
                <>
                  ...
                  <span style={{ fontSize: '13px', background: '#fef2f2', color: '#ef4444', padding: '3px 10px', borderRadius: '20px', fontWeight: '600', display: 'inline-block' }}>
                    {categories.find(c => c.value === entry.category)?.label || entry.category}
                  </span>
                  <span style={{ fontSize: '13px', color: '#64748b' }}>{entry.description || '—'}</span>
                  <span style={{ fontWeight: '700', color: '#ef4444', fontSize: '15px' }}>₪{Number(entry.amount).toLocaleString()}</span>
