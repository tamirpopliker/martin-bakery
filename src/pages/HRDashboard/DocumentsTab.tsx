...
export function DocumentsTab({ employee }: { employee: UnifiedEmployee }) {
  ...
  async function loadAll() {
    setLoading(true)
    const [allTypes, docsRes] = await Promise.all([
      loadDocumentTypes(),
    ...
    // Permanent documents only. Monthly events (sick notes, resignation, etc.)
    // live in MonthlyEventsTab and are excluded here by their type flag.
    const permanentTypes = allTypes.filter(t => !t.is_monthly_event)
    const permanentTypeIds = new Set(permanentTypes.map(t => t.id))
    const allDocs = (docsRes.data as EmployeeDocument[]) || []
    setDocTypes(permanentTypes)
    setDocs(allDocs.filter(d => d.document_type_id != null && permanentTypeIds.has(d.document_type_id)))
    if (permanentTypes.length > 0 && selectedType === null) setSelectedType(permanentTypes[0].id)
    setLoading(false)
  }
