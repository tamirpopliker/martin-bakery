...
function buildSystemPrompt(userName: string, ctx: AgentContext, todayFallback: string): string {
  ...
  lines.push(
    ...
    '',
    '**כתיבה — פחת** (`add_branch_waste`) **והוצאות** (`add_branch_expense`).',
    '',
    ...
    '',
    'כל שאר הרישומים — סגירת קופה, קופת עודף, הזמנות עוגות — **עדיין לא זמינים**. אם מתבקשים, אמור זאת בפשטות ואל תעמיד פנים שביצעת משהו.',
  )
  return lines.join('\n')
}
